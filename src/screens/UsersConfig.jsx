  ...
  return (
    <div data-screen-label="13 Admin users and config" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, minHeight: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <span style={{ fontFamily: MONO, fontSize: 10, letterSpacing: '0.12em', padding: '4px 10px', borderRadius: 999, background: 'rgba(var(--surf-rgb),0.5)', backdropFilter: 'blur(14px)', border: '1px solid rgba(var(--surf-rgb),0.65)', color: 'var(--mute-2)' }}>ADMIN ONLY</span>
      </div>
      ...
      <div className="config-grid">
        <div style={{ ...glass, padding: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <span style={{ fontSize: 12, color: 'var(--mute)' }}>One Operator role covers Pack · Receive · Returns. Finer roles split out as the team grows.</span>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ ...glass, padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontSize: 14, color: 'var(--mute-2)', flex: 1 }}>Retention window</span>
              <span style={{ fontFamily: MONO, fontSize: 14 }}>365 days</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontSize: 14, color: 'var(--mute-2)', flex: 1 }}>S3 tiering (hot → archive)</span>
              <button
                onClick={() => set({ tiering: !s.tiering })}
                style={{ cursor: 'pointer', borderRadius: 999, padding: '6px 14px', fontSize: 12, fontWeight: 700, letterSpacing: '0.06em', border: '1px solid ' + (s.tiering ? 'rgba(23,163,95,0.5)' : 'rgba(0,0,0,0.2)'), background: s.tiering ? 'rgba(23,163,95,0.08)' : 'transparent', color: s.tiering ? '#0E8A50' : 'var(--mute)' }}
              >
            ...
            <span style={{ fontSize: 12, color: 'var(--mute)' }}>Footage past retention is auto-deleted.</span>
          </div>
