  ...
  return (
    <div data-screen-label="02 Kiosk home" style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, padding: 28, overflow: 'auto' }}>
      ...
      <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 760 }}>
        ...
        <div style={{ fontSize: 16, color: 'var(--mute-2)', textWrap: 'pretty' }}>Pick a task — Pack, Receive or Returns — then scan. The session opens and recording starts.</div>
      </div>
        ...
        <div className="demo-chip-grid">
          {demoChips.map((d) => (
            <button
              ...
              style={{ textAlign: 'left', background: 'rgba(var(--surf-rgb),0.45)', border: '1px solid rgba(var(--surf-rgb),0.55)', borderRadius: 16, padding: '13px 14px', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 4 }}
            >
              <span style={{ fontFamily: MONO, fontSize: 14, fontWeight: 500, color: 'var(--ink-2)' }}>{d.label}</span>
              <span style={{ fontSize: 12, color: 'var(--mute)' }}>{d.sub}</span>
            </button>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
        <div style={{ fontFamily: MONO, fontSize: 12, color: 'var(--mute)' }}>Last session: {s.lastSession}</div>
        <button className="hv-text-mid" onClick={signOut} style={{ background: 'none', border: 'none', color: 'var(--mute)', fontFamily: MONO, fontSize: 11, cursor: 'pointer', letterSpacing: '0.08em' }}>
          ADMIN EXIT (PIN)
        </button>
      </div>
    </div>
