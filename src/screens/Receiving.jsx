    ...
    <div data-screen-label="04 Store receiving" className="recv-grid">
        ...
        <div style={{ ...glass, padding: 16, display: 'flex', flexDirection: 'column', gap: 10, flex: 1, minHeight: 0, overflow: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <span style={{ fontFamily: MONO, fontSize: 12, color: 'var(--ink-50)' }}>Challan {s.recvChallan}</span>
            </div>
            <span style={{ fontFamily: MONO, fontSize: 10, letterSpacing: '0.12em', padding: '4px 10px', borderRadius: 999, background: 'var(--tile)', color: 'var(--ink-55)' }}>GATI · CHALLAN</span>
          </div>
          {rows.map((row) => (
            <div key={row.key} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--tile)', border: '1px solid var(--tile-border)', borderRadius: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                <span style={{ fontSize: 13, color: 'var(--ink-55)' }}>{row.name}</span>
              </div>
          ...
          <div style={{ fontFamily: MONO, fontSize: 13, color: 'var(--ink-70)', padding: '10px 14px', background: 'var(--tile)', borderRadius: 12, border: '1px solid var(--tile-border)' }}>{summary}</div>
        </div>
      ...
      <div style={{ ...glass, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 18px', borderBottom: '1px solid var(--line)' }}>
          ...
          <span style={{ fontFamily: MONO, fontSize: 14, color: 'var(--ink)' }}>Arrival · {s.recvChallan}</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '0 13px 13px' }}>
          <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--ink-40)' }}>arrival video retained with the pack video · both attach to any flag</span>
        </div>
      </div>
    </div>
  );
