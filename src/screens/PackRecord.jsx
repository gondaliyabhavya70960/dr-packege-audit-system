    ...
    <div data-screen-label="03 Pack and Record" className="pack-grid">
      ...
      <div style={{ ...glass, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 18px', borderBottom: '1px solid var(--line)' }}>
          ...
          <span style={{ fontFamily: MONO, fontSize: 14, color: 'var(--ink)' }}>Session {s.packId}</span>
          <span style={{ marginLeft: 'auto', fontFamily: MONO, fontSize: 17, color: 'var(--ink)' }}>{fmt(s.recSec)}</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '0 13px 13px' }}>
          <button className="hv-border-accent" onClick={() => set({ packStills: s.packStills + 1 })} style={{ background: 'var(--tile)', border: '1px solid var(--line)', color: 'var(--ink)', borderRadius: 10, padding: '10px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
            📷 Capture still
          </button>
          <span style={{ fontFamily: MONO, fontSize: 12, color: 'var(--ink-55)' }}>stills: {s.packStills}</span>
          <span style={{ marginLeft: 'auto', fontFamily: MONO, fontSize: 11, color: 'var(--ink-40)' }}>chunks hashed at capture · uploading</span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minHeight: 0 }}>
        <div style={{ ...glass, padding: 16, display: 'flex', flexDirection: 'column', gap: 10, flex: 1, minHeight: 0, overflow: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
            ...
            <span style={{ fontFamily: MONO, fontSize: 10, letterSpacing: '0.12em', padding: '4px 10px', borderRadius: 999, background: 'var(--tile)', color: 'var(--ink-55)' }}>GATI · LIVE</span>
          </div>
          {rows.map((row) => (
            <div key={row.key} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--tile)', border: '1px solid var(--tile-border)', borderRadius: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 1, minWidth: 0 }}>
                ...
                <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--ink-50)' }}>{row.sku}</span>
              </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            {unknown && (
              <button onClick={() => set({ packUnknown: false })} style={{ background: 'var(--tile)', border: '1px solid var(--line-strong)', color: 'var(--ink)', borderRadius: 999, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          ...
          {allScanned && !unknown && s.packCond !== 'confirmed' && (
            <div style={{ border: '1px solid rgba(217,142,4,0.35)', background: 'rgba(217,142,4,0.06)', borderRadius: 16, padding: 13, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <span style={{ fontSize: 14, color: 'var(--ink-70)', lineHeight: 1.45, textWrap: 'pretty' }}>
