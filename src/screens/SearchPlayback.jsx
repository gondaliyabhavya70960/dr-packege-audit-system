  ...
  return (
    <div data-screen-label="06 Search and playback" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, minHeight: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <span style={{ fontFamily: MONO, fontSize: 10, letterSpacing: '0.12em', padding: '4px 10px', borderRadius: 999, background: 'var(--tile)', backdropFilter: 'blur(14px)', border: '1px solid var(--tile-border)', color: 'var(--ink-55)' }}>READ-ONLY AUDIT</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        <input
          ...
          style={{ flex: 1, maxWidth: 520, background: 'var(--tile)', backdropFilter: 'blur(14px)', border: '1px solid var(--tile-border)', borderRadius: 10, padding: '12px 20px', color: 'var(--ink)', fontSize: 15, outline: 'none', boxShadow: '0 2px 10px rgba(15,30,60,0.04)' }}
        />
        <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--ink-45)' }}>Filter: date / station / outcome</span>
      </div>
      ...
      <div className="search-grid">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {results.map((r) => {
            ...
            return (
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 12, textAlign: 'left', cursor: 'pointer', background: 'var(--tile-strong)', backdropFilter: 'blur(22px) saturate(1.4)', borderRadius: 18, padding: '13px 15px', border: '1px solid ' + (active ? 'rgba(142,14,34,0.6)' : 'var(--line)'), boxShadow: '0 2px 8px rgba(15,30,60,0.04)' }}
              >
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0, flex: 1 }}>
                  <span style={{ fontFamily: MONO, fontSize: 14, color: 'var(--ink)' }}>{r.id}</span>
                  <span style={{ fontSize: 13, color: 'var(--ink-55)' }}>{r.kinds}</span>
                </div>
          ...
          {results.length === 0 && (
            <div style={{ padding: 24, textAlign: 'center', color: 'var(--ink-45)', fontSize: 14, border: '1px dashed var(--line-strong)', borderRadius: 16 }}>
            ...
            <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <button className="hv-border-accent" onClick={() => openOrder(sel.id)} style={{ background: 'var(--tile-strong)', border: '1px solid var(--tile-border)', color: '#8E0E22', borderRadius: 999, padding: '9px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  <span style={{ fontSize: 12, color: 'var(--ink-45)' }}>Operator</span>
                  <span style={{ fontSize: 14, fontWeight: 600 }}>{sel.operator}</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  <span style={{ fontSize: 12, color: 'var(--ink-45)' }}>Station</span>
                  <span style={{ fontFamily: MONO, fontSize: 13 }}>{sel.station}</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  <span style={{ fontSize: 12, color: 'var(--ink-45)' }}>Timestamp</span>
                  <span style={{ fontFamily: MONO, fontSize: 13 }}>{sel.ts}</span>
                </div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, background: 'var(--tile)', border: '1px solid var(--tile-border)', borderRadius: 12, padding: '11px 14px' }}>
                <span style={{ fontSize: 12, color: 'var(--ink-45)' }}>File hash — tamper evidence (re-verified on arrival)</span>
