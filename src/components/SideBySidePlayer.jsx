...
export default function SideBySidePlayer({ ctx }) {
  ...
  const clipPane = (label, meta, text) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: MONO, fontSize: 11, color: 'var(--ink-55)' }}>
    ...
    <div data-screen-label="07 Side-by-side player" style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(28,20,32,0.32)', backdropFilter: 'blur(16px) saturate(1.4)', WebkitBackdropFilter: 'blur(16px) saturate(1.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 30 }}>
      <div style={{ ...glassSheet, borderRadius: 28, width: 1040, maxWidth: '96%', maxHeight: '94vh', overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px', borderBottom: '1px solid var(--line)' }}>
          ...
          <button className="hv-white75" onClick={closePlayer} style={{ marginLeft: 'auto', background: 'var(--tile)', border: 'none', color: 'var(--ink-70)', borderRadius: '50%', width: 34, height: 34, fontSize: 15, cursor: 'pointer' }}>
            ✕
          </button>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px' }}>
          <button className="hv-white75" onClick={() => set({ t: Math.max(0, s.t - 10) })} style={{ background: 'var(--tile)', border: 'none', color: 'var(--ink)', borderRadius: 999, padding: '8px 14px', fontSize: 13, cursor: 'pointer' }}>
            ◀◀
          </button>
          ...
          <button className="hv-white75" onClick={() => set({ t: Math.min(100, s.t + 10) })} style={{ background: 'var(--tile)', border: 'none', color: 'var(--ink)', borderRadius: 999, padding: '8px 14px', fontSize: 13, cursor: 'pointer' }}>
            ▶▶
          </button>
          ...
          <span style={{ fontFamily: MONO, fontSize: 13, color: 'var(--ink-60)' }}>{fmt(Math.round(s.t * 0.84))} / 01:24</span>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '0 20px 14px', flexWrap: 'wrap' }}>
          <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--ink-50)' }}>STILLS</span>
          {STILL_LABELS.map((l, i) => {
            ...
            return (
              <button
                ...
                style={{ cursor: 'pointer', borderRadius: 999, padding: '9px 16px', fontSize: 13, fontWeight: 600, background: 'var(--tile)', border: '1px solid ' + (active ? 'rgba(142,14,34,0.7)' : 'var(--line)'), color: active ? '#8E0E22' : 'var(--ink-60)' }}
              >
                {active ? l + ' · enlarged' : l}
              </button>
            );
          })}
          <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--ink-35)' }}>click a still to enlarge</span>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', borderTop: '1px solid var(--line)', flexWrap: 'wrap' }}>
          <span style={{ fontSize: 14, color: 'var(--ink-60)' }}>{fromFlag ? 'Supervisor decision — releases or keeps the refund hold.' : 'Verdict feeds the accept / flag decision.'}</span>
          <div style={{ flex: 1 }} />
