...
import { Play, Pause, X, ChevronRight } from './line-icons.jsx';
    ...
    return (
      ...
      >
        <span style={{ position: 'relative', width: 104, height: 62, flex: 'none', borderRadius: 10, overflow: 'hidden', ...feedBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <span style={{ width: 26, height: 26, borderRadius: '50%', background: 'rgba(var(--accent-rgb),0.92)', color: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Play size={12} aria-hidden="true" /></span>
        </span>
  ...
  return (
    <div style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: 'rgba(0,0,0,0.62)' }}>
        <button onClick={() => setPlaying((p) => !p)} aria-label={playing ? 'Pause' : 'Play'} style={{ flex: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'var(--accent)', color: '#FFFFFF', border: 'none', borderRadius: 999, padding: '6px 15px', cursor: 'pointer' }}>{playing ? <Pause size={13} aria-hidden="true" /> : <Play size={13} aria-hidden="true" />}</button>
        <input type="range" min="0" max="100" value={t} onChange={(e) => setT(Number(e.target.value))} style={{ flex: 1, accentColor: 'var(--accent)', cursor: 'pointer' }} />
